...
from WirelessEnv import AoIEnv_IID_Unreliable
from DeepTOP import DeepTOP_RMAB
from Whittle_IID_Unreliable import Whittle_IID_Unreliable


def initializeEnv():
    global envs, state_dims, action_dims, nb_arms, global_seed
    for i in range(nb_arms):
        envs.append(AoIEnv_IID_Unreliable(seed=global_seed + i*1000, p=0.2 + 0.6/nb_arms*i))
        state_dims.append(1)
...
if __name__ == '__main__':
    ...
    # 0: DeepTOP; 1: Whittle_IID_Unreliable
    ...
    if agent_policy == 0:
        ...
        agent = Whittle_IID_Unreliable(nb_arms, budget, state_dims, action_dims, hidden, args)
    agent.eval()
    resetEnvs()
    agent.reset(states)

...
from LPQL import LPQL


def initializeEnv():
    global envs, state_dims, action_dims, state_sizes, action_sizes, nb_arms
    for i in range(nb_arms):
        envs.append(lineEnv(seed=i*1000, N=100, OptX=20 + i*(60.0/(nb_arms-1)), p=1, q=1))
        state_dims.append(1)
...
if __name__ == '__main__':
    ...
    #initialize agent
    hidden = [8, 16, 16, 8]
    agent = LPQL(nb_arms, budget, state_dims, action_dims, state_sizes, action_sizes, hidden, args)
    agent.eval()
    resetEnvs()
    agent.reset(states)

...
class DeepTOP_RMAB(object):
    ...
    def __init__(self, nb_arms, budget, state_dims, action_dims, state_sizes, action_sizes, hidden, args):
        ...
        for arm in range(nb_arms):
            ...
            self.actor_targets.append(Actor(self.state_dims[arm], 1, hidden))
            ...
            hard_update(self.actor_targets[arm], self.actors[arm])
    ...
    def update_policy(self):
        for arm in range(self.nb_arms):
            ...
            for i in range(self.batch_size):
                ...
                if self.critic_targets[arm]([torch.FloatTensor(next_state_batch[i]), torch.FloatTensor([price_batch[i]]),
                                             torch.FloatTensor([1])]) \
            ...
            q_diff_batch = []
            for i in range(self.batch_size):
                q_diff_batch.append(self.critics[arm]([state_batch[i],
                                                       self.actors[arm](state_batch[i]),
                                                       torch.FloatTensor([1])]).item() - \
                                    self.critics[arm]([state_batch[i],
                                                       self.actors[arm](state_batch[i]),
                                                       torch.FloatTensor([0])]).item())
            q_diff_batch = torch.FloatTensor(q_diff_batch).unsqueeze(dim=-1)
            policy_loss = -q_diff_batch * self.actors[arm](state_batch)
            policy_loss = policy_loss.mean()
            policy_loss.backward()
            self.actor_optims[arm].step()

            # Target update
            soft_update(self.actor_targets[arm], self.actors[arm], self.tau)
            soft_update(self.critic_targets[arm], self.critics[arm], self.tau)

    def eval(self):
        for arm in range(self.nb_arms):
            self.actors[arm].eval()
            self.actor_targets[arm].eval()
    ...
    def select_action(self, s_t, decay_epsilon=True):
        indices = []
        for arm in range(self.nb_arms):
            indices.append(self.actors[arm].forward(torch.FloatTensor(self.s_t[arm])).detach().numpy()[0])
        sort_indices = indices.copy()
        sort_indices.sort(reverse=True)
        sort_indices.append(
